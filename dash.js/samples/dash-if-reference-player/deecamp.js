...
app.controller('DashController', function ($scope, sources, contributors, dashifTestVectors) {
    ...
    $scope.videoCopy =  document.querySelector('.player-copy video');
    ...
    window.playerCopy = $scope.playerCopy = dashjs.MediaPlayer().create(); /* jshint ignore:line */
    ////////////////////////////////////////
    //
    // Configuration file
    //
    ...
    reqConfig.onload = function() {
        if (reqConfig.status === 200) {
            ...
            if ($scope.playerCopy) {
                $scope.playerCopy.updateSettings(config);
            }
        }
    };
    ...
    var playerList = [$scope.player, $scope.playerCopy]
    playerList.forEach( function(item) { /* jshint ignore:line */
        item.on(dashjs.MediaPlayer.events.ERROR, function (e) { /* jshint ignore:line */
            if (!e.event) {
    ...
    $scope.playerCopy.initialize($scope.videoCopy, null, $scope.autoPlaySelected);
    ...
    $scope.playerCopy.updateSettings(initialConfig);
    ...
    $scope.controlbarCopy = new ControlBar($scope.playerCopy); /* jshint ignore:line */
    $scope.controlbarCopy.initialize(2); // id后面都跟2
    $scope.controlbarCopy.disable();

    playerList.forEach(function(item) { /* jshint ignore:line */
        item.on(dashjs.MediaPlayer.events.MANIFEST_LOADED, function (e) { /* jshint ignore:line */
            $scope.isDynamic = e.data.type === 'dynamic';
        }, $scope);
    })

    playerList.forEach(function(item) { /* jshint ignore:line */
        item.on(dashjs.MediaPlayer.events.QUALITY_CHANGE_REQUESTED, function (e) { /* jshint ignore:line */
    ...
    playerList.forEach(function(item) { /* jshint ignore:line */
        item.on(dashjs.MediaPlayer.events.QUALITY_CHANGE_RENDERED, function (e) { /* jshint ignore:line */
    ...
    playerList.forEach(function(item) { /* jshint ignore:line */
        item.on(dashjs.MediaPlayer.events.PERIOD_SWITCH_COMPLETED, function (e) { /* jshint ignore:line */
            $scope.streamInfo = e.toStreamInfo;
        }, $scope);
    })

    playerList.forEach(function(item) { /* jshint ignore:line */
        item.on(dashjs.MediaPlayer.events.STREAM_INITIALIZED, function (e) { /* jshint ignore:line */
            stopMetricsInterval();
    ...
    playerList.forEach(function(item) { /* jshint ignore:line */
        item.on(dashjs.MediaPlayer.events.PLAYBACK_ENDED, function (e) { /* jshint ignore:line */
    ...
    playerList.forEach(function(item) { /* jshint ignore:line */
        item.on(dashjs.MediaPlayer.events.KEY_SYSTEM_SELECTED, function (e) { /* jshint ignore:line */
            if (e.data) {
    ...
    playerList.forEach(function(item) { /* jshint ignore:line */
        item.on(dashjs.MediaPlayer.events.KEY_SESSION_CREATED, function (e) { /* jshint ignore:line */
            if (e.data) {
                var session = e.data;
    ...
    $scope.doLoad = function () {
        ...
        $scope.controlbarCopy.reset();
        $scope.player.setProtectionData(protData);
        $scope.player.attachSource($scope.selectedItem.url);
        $scope.playerCopy.setProtectionData(protData);
        $scope.playerCopy.attachSource($scope.selectedItem.url); // 将来在这里替换url
        ...
        if ($scope.initialSettings.video) {
            ...
            $scope.playerCopy.setInitialMediaSettingsFor('video', {
                role: $scope.initialSettings.video
            });
        }
        if ($scope.initialSettings.text) {
            $scope.player.setTextDefaultLanguage($scope.initialSettings.text);
            $scope.playerCopy.setTextDefaultLanguage($scope.initialSettings.text);
        }
        ...
        $scope.playerCopy.setTextDefaultEnabled($scope.initialSettings.textEnabled);
        $scope.playerCopy.enableForcedTextStreaming($scope.initialSettings.forceTextStreaming);
        $scope.controlbar.enable();
        $scope.controlbarCopy.enable();
    };

    $scope.doStop = function () {
        $scope.player.attachSource(null);
        $scope.playerCopy.attachSource(null);
        $scope.controlbar.reset();
        $scope.controlbarCopy.reset();
        stopMetricsInterval();
    };
    ...
    $scope.clearChartData = function () {
        ...
        for (var key in $scope.chartState2) {
            for (var i in $scope.chartState2[key]) {
                $scope.chartState2[key][i].data.length = 0;
            }
        }
    };
    ...
    $scope.plotPoint2 = function (name, type, value, time) {
        if ($scope.chartEnabled) {
            var specificChart2 = $scope.chartState2[type];
            if (specificChart2) {
                var data = specificChart2[name].data;
                data.push([time, value]);
                if (data.length > $scope.maxPointsToChart) {
                    data.splice(0, 1);
                }
    ...
    $scope.enableChartByName = function (id, type) {
        ...
        if ($scope.chartState[type][id].selected) {
            ...
            var data2 = {
                id: id,
                data: $scope.chartState2[type][id].data,
                label: $scope.chartState2[type][id].label,
                color: $scope.chartState2[type][id].color,
                yaxis: $scope.chartData2.length + 1,
                type: type
            };
            $scope.chartData.push(data);
            $scope.chartData2.push(data2);
    ...
    function updateMetrics2(type) {
        var dashMetrics = $scope.playerCopy.getDashMetrics();
        var dashAdapter = $scope.playerCopy.getDashAdapter();
        ...
        if (dashMetrics && $scope.streamInfo) {
            ...
            $scope[type + 'BufferLength2'] = bufferLevel;
            $scope[type + 'MaxIndex2'] = maxIndex;
            $scope[type + 'Bitrate2'] = bitrate;
            $scope[type + 'DroppedFrames2'] = droppedFPS;
            $scope[type + 'LiveLatency2'] = liveLatency;

            var httpMetrics = calculateHTTPMetrics(type, dashMetrics.getHttpRequests(type));
            if (httpMetrics) {
                $scope[type + 'Download2'] = httpMetrics.download[type].low.toFixed(2) + ' | ' + httpMetrics.download[type].average.toFixed(2) + ' | ' + httpMetrics.download[type].high.toFixed(2);
                $scope[type + 'Latency2'] = httpMetrics.latency[type].low.toFixed(2) + ' | ' + httpMetrics.latency[type].average.toFixed(2) + ' | ' + httpMetrics.latency[type].high.toFixed(2);
                $scope[type + 'Ratio2'] = httpMetrics.ratio[type].low.toFixed(2) + ' | ' + httpMetrics.ratio[type].average.toFixed(2) + ' | ' + httpMetrics.ratio[type].high.toFixed(2);
            }

            if ($scope.chartCount2 % 2 === 0) {
                var time = getTimeForPlot();
                $scope.plotPoint2('buffer', type, bufferLevel, time);
                $scope.plotPoint2('index', type, index, time);
                $scope.plotPoint2('bitrate', type, bitrate, time);
                $scope.plotPoint2('droppedFPS', type, droppedFPS, time);
                $scope.plotPoint2('liveLatency', type, liveLatency, time);

                if (httpMetrics) {
                    $scope.plotPoint2('download', type, httpMetrics.download[type].average.toFixed(2), time);
                    $scope.plotPoint2('latency', type, httpMetrics.latency[type].average.toFixed(2), time);
                    $scope.plotPoint2('ratio', type, httpMetrics.ratio[type].average.toFixed(2), time);
                }
                $scope.safeApply();
            }
        }
