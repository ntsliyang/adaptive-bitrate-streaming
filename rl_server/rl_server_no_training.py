...
# NN_MODEL = None
LINEAR_NN_MODEL = '../rl_server/results/pretrain_linear_reward.ckpt'
LOG_NN_MODEL = '../rl_server/results/log_scale_reward.ckpt'
HD_NN_MODEL = '../rl_server/results/hd_reward.ckpt'
...
def make_request_handler(input_dict):
    ...
    class Request_Handler(BaseHTTPRequestHandler):
        ...
        def set_param(self, type):
            print("========================================================================")
            if type == 'linear':
                self.input_dict['saver'].restore(self.input_dict['sess'], LINEAR_NN_MODEL)
                print('Linear reward model restored.')
            elif type == 'log':
                self.input_dict['saver'].restore(self.input_dict['sess'], LOG_NN_MODEL)
                print('Log scale reward model restored.')
            elif type == 'hd':
                self.input_dict['saver'].restore(self.input_dict['sess'], HD_NN_MODEL)
                print('HD reward model restored.')
        ...
        def do_POST(self):
            ...
            if ( 'pastThroughput' in post_data ):
                ...
                # reward = None
                if ('type' in post_data == False or post_data['type'] == 'linear'):
                ...
                # update input dict according to user choice of RL model
                self.set_param(post_data['type'])
...
def run(server_class=HTTPServer, port=8333, log_file_path=LOG_FILE):
    ...
    with tf.Session() as sess, open(log_file_path, 'w') as log_file:
        ...
        if LINEAR_NN_MODEL is not None:  # nn_model is the path to file
            saver.restore(sess, LINEAR_NN_MODEL)
...
def main():
    if len(sys.argv) == 2:
        ...
        # run(log_file_path="./results/log/log_sim_rl_norway_bus_1")
